...
let Seeds = class Seeds {
    constructor(authRepository, configService) {
        this.authRepository = authRepository;
        this.configService = configService;
    }
    async onModuleInit() {
        const isAdmin = await this.authRepository.findOne({
            where: {
                role: Role.ADMIN,
            },
        });
        const firstSeed = this.admSeed.admSeed();
        if (!isAdmin) {
            const admin = this.authRepository.create(firstSeed);
